...
scene.add(root);
scene.add(new THREE.HemisphereLight(0xb8edff, 0x06101a, 1.3));
...
loader.load("./assets/textures/sphere-ice.png", t => textures[0].image=t.image, undefined, () => {});
loader.load("./assets/textures/sphere-lava.png", t => textures[1].image=t.image, undefined, () => {});
loader.load("./assets/textures/sphere-emerald.png", t => textures[2].image=t.image, undefined, () => {});
loader.load("./assets/textures/sphere-amber.png", t => textures[3].image=t.image, undefined, () => {});

makeSky(); resetGame(false); connect(); animate();

function makeActor(id, label, color, bulletColor) {
  const g = new THREE.Group();
  const mat = new THREE.MeshStandardMaterial({ color, emissive:color, emissiveIntensity:.22, metalness:.45, roughness:.25 });
...
function makeSky(){
  const sky = new THREE.Mesh(new THREE.SphereGeometry(190,64,32), new THREE.MeshBasicMaterial({ map:skyTexture(), color:0x9bdfff, side:THREE.BackSide, fog:false })); scene.add(sky);
  const geo = new THREE.BufferGeometry(), pts = new Float32Array(2200*3);
  for(let i=0;i<2200;i++){ const r=110+Math.random()*150,a=Math.random()*Math.PI*2,z=Math.random()*2-1,h=Math.sqrt(1-z*z); pts[i*3]=Math.cos(a)*h*r; pts[i*3+1]=z*r; pts[i*3+2]=Math.sin(a)*h*r; }
...
function resetGame(sendRestart){
  ...
  platforms.length=enemies.length=bullets.length=effects.length=0; scores.p1=scores.p2=0; state.started=false; state.ended=false; state.msg="برای شروع کلیک کنید"; overlay.classList.add("show");
  ...
  [[1,.2,1],[2,-.2,2.4],[3,.1,4],[4,-.3,1.5],[5,.2,4.8],[6,-.1,2.8]].forEach(s=>makeEnemy(...s)); updateUi(); if(sendRestart) send({type:"restart"});
}
function makePlatform([radius,base,amp,glow],i){ const group=new THREE.Group(); const mat=new THREE.MeshStandardMaterial({color:0xffffff,map:textures[i%4],roughness:.42,metalness:.32,emissive:glow,emissiveIntensity:.16}); group.add(new THREE.Mesh(new THREE.SphereGeometry(radius,42,42),mat)); const ring=new THREE.Mesh(new THREE.TorusGeometry(radius*1.22,radius*.08,12,72),new THREE.MeshBasicMaterial({color:glow,transparent:true,opacity:.48})); ring.rotation.set(Math.random()*Math.PI,Math.random()*Math.PI,0); group.add(ring); root.add(group); return {group,mesh:group,radius,base:new THREE.Vector3(...base),amp:new THREE.Vector3(...amp),center:new THREE.Vector3(...base),prev:new THREE.Vector3(...base),delta:new THREE.Vector3(),phase:Math.random()*6,speed:.34+i*.04,ring,mat}; }
function makeEnemy(pi,lat,lon){ const mesh=new THREE.Group(), red=new THREE.MeshStandardMaterial({color:0xff2525,emissive:0x5a0505,emissiveIntensity:.55}); mesh.add(new THREE.Mesh(new THREE.CapsuleGeometry(.48,1,4,10),red)); const eye=new THREE.Mesh(new THREE.SphereGeometry(.22,16,16),new THREE.MeshBasicMaterial({color:0xffeeee})); eye.position.set(0,.62,.34); mesh.add(eye); root.add(mesh); enemies.push({mesh,platform:platforms[pi],lat,lon,health:60,cool:.8+Math.random(),pos:new THREE.Vector3(),dead:false}); }
function place(p,platform,up){ p.platform=platform; p.up.copy(up); p.pos.copy(platform.center).addScaledVector(up, platform.radius+.9); p.vel.set(0,0,0); p.health=100; p.alive=true; sync(p); }
function start(){ if(state.ended) resetGame(true); state.started=true; state.msg="رقابت شروع شد"; overlay.classList.remove("show"); updateUi(); }
...
function update(dt){ const t=clock.elapsedTime; for(const p of platforms){ p.prev.copy(p.center); p.center.set(p.base.x+Math.sin(t*p.speed+p.phase)*p.amp.x,p.base.y+Math.cos(t*p.speed*1.5+p.phase)*p.amp.y,p.base.z+Math.sin(t*p.speed*.8+p.phase)*p.amp.z); p.delta.subVectors(p.center,p.prev); p.group.position.copy(p.center); p.group.rotation.y+=dt*.28; p.ring.rotation.x+=dt*.35; }
 if(state.started&&!state.ended){ updateLocal(local(),dt); updateRemote(remote(),dt); updateEnemies(dt); updateBullets(dt); sendState(); } updateEffects(dt); updateCamera(dt); updateUi(); }
function updateLocal(p,dt){ if(!p||!p.alive)return; p.cool=Math.max(0,p.cool-dt); if(p.grounded&&p.platform)p.pos.add(p.platform.delta); const plat=nearest(p.pos), up=p.pos.clone().sub(plat.center).normalize(); p.up.lerp(up,1-Math.exp(-12*dt)).normalize(); const f=p.forward.clone().projectOnPlane(p.up).normalize(), r=new THREE.Vector3().crossVectors(f,p.up).normalize(), m=new THREE.Vector3(); if(input.f)m.add(f); if(input.b)m.sub(f); if(input.r)m.add(r); if(input.l)m.sub(r); if(m.lengthSq()>.01)m.normalize(); const ns=p.vel.dot(p.up), tan=p.vel.clone().sub(p.up.clone().multiplyScalar(ns)); tan.addScaledVector(m,(p.grounded?70:24)*dt); if(tan.length()>24)tan.setLength(24); if(p.grounded)tan.multiplyScalar(Math.exp(-2.7*dt)); p.vel.copy(tan).addScaledVector(p.up,ns); if(input.jump&&p.grounded){p.vel.addScaledVector(p.up,18);p.vel.addScaledVector(m.lengthSq()?m:f,11);p.grounded=false;} input.jump=false; p.vel.addScaledVector(p.up,-31*dt); p.pos.addScaledVector(p.vel,dt); land(p); if(input.fire&&p.cool<=0)fire(p); if(m.lengthSq()>.01)p.forward.copy(m); sync(p); }
...
function updateBullets(dt){ for(let i=bullets.length-1;i>=0;i--){ const b=bullets[i]; b.life-=dt; b.pos.addScaledVector(b.vel,dt); b.mesh.position.copy(b.pos); if(b.life<=0||platforms.some(p=>b.pos.distanceTo(p.center)<p.radius+.16)){removeBullet(i);continue;} if(b.owner==="enemy"){ const p=local(); if(p&&p.alive&&b.pos.distanceTo(p.pos)<1.15){p.health=Math.max(0,p.health-15);send({type:"damage",target:p.id,health:p.health});removeBullet(i);} continue; } for(let ei=0;ei<enemies.length;ei++){ const e=enemies[ei]; if(!e.dead&&b.pos.distanceTo(e.pos)<1.25){ e.health-=34; if(e.health<=0){e.dead=true;root.remove(e.mesh);scores[b.owner]++;send({type:"enemy-down",index:ei,scorer:b.owner});} removeBullet(i); break; } } } }
function fire(p){ const dir=p.forward.clone().normalize(), muzzle=p.pos.clone().addScaledVector(p.up,.72).addScaledVector(p.forward,1); shoot(muzzle,dir,72,p.id,p.bulletColor); p.cool=.15; send({type:"shot",origin:pack(muzzle),direction:pack(dir),color:p.bulletColor}); }
...
function updateCamera(dt){ const p=local()||players.p1, up=p.up.clone().normalize(), f=p.forward.clone().projectOnPlane(up).normalize(), target=p.pos.clone().addScaledVector(up,2.3).addScaledVector(f,1.5), desired=target.clone().addScaledVector(f,-13).addScaledVector(up,4); camera.position.lerp(desired,1-Math.exp(-8*dt)); camera.up.copy(up); camera.lookAt(target); }
