...
function patchGameScript(source) {
  return source
    ...
    .replace(
      ...
      'startBtn?.addEventListener("click",start);const tj=document.getElementById("touch-jump"),tf=document.getElementById("touch-fire"),ts=document.getElementById("touch-stick"),kn=document.getElementById("touch-stick-knob");tj?.addEventListener("pointerdown",()=>{input.jump=true;start();});tf?.addEventListener("pointerdown",()=>{input.fire=true;start();});tf?.addEventListener("pointerup",()=>input.fire=false);if(ts){let pid=null;const reset=()=>{input.f=input.b=input.l=input.r=false;pid=null;if(kn)kn.style.transform="translate(-50%, -50%)";ts.classList.remove("active")},move=p=>{const r=ts.getBoundingClientRect(),cx=r.left+r.width/2,cy=r.top+r.height/2,dx=p.clientX-cx,dy=p.clientY-cy,mx=r.width*.38,len=Math.min(mx,Math.hypot(dx,dy)),a=Math.atan2(dy,dx),nx=Math.cos(a)*len/mx,ny=Math.sin(a)*len/mx;input.f=ny<-.2;input.b=ny>.2;input.l=nx<-.2;input.r=nx>.2;if(kn)kn.style.transform=`translate(calc(-50% + ${nx*mx}px), calc(-50% + ${ny*mx}px))`;};ts.addEventListener("pointerdown",e=>{e.preventDefault();pid=e.pointerId;ts.setPointerCapture?.(e.pointerId);ts.classList.add("active");start();move(e);});ts.addEventListener("pointermove",e=>{if(pid!==null&&e.pointerId!==pid)return;e.preventDefault();move(e);});ts.addEventListener("pointerup",reset);ts.addEventListener("pointercancel",reset);ts.addEventListener("lostpointercapture",reset);ts.addEventListener("touchstart",e=>{e.preventDefault();ts.classList.add("active");start();move(e.touches[0]);},{passive:false});ts.addEventListener("touchmove",e=>{e.preventDefault();if(e.touches[0])move(e.touches[0]);},{passive:false});ts.addEventListener("touchend",reset);ts.addEventListener("touchcancel",reset);}let lookId=null,lx=0;canvas.addEventListener("pointerdown",e=>{if(e.pointerType==="mouse"||e.clientX<innerWidth*.38)return;lookId=e.pointerId;lx=e.clientX;start();});canvas.addEventListener("pointermove",e=>{const p=local();if(e.pointerId!==lookId||!p||!state.started)return;const dx=e.clientX-lx;lx=e.clientX;cam.forward.applyAxisAngle(p.up,-dx*.0052);});canvas.addEventListener("pointerup",e=>{if(e.pointerId===lookId)lookId=null;});'
    );
}
